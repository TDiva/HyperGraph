...
public class Visualizator {
    ...
    // заполняет таблицу инцидентности графа
    ...
    // заполняет таблицу инцидентности подграфа
    ...
    // заполняет таблицу инцидентности стяжки графа
    ...
    private static void drawEdge(Graphics g, int edge, int index, int v) {
        ...
        if (deg == 2) {
            ...
            for (int i = 0; i < row.size() - 1; i++) {
                ...
                int d = (int) Math.sqrt(Math.pow(x2 - x1, 2d) + Math.pow(y2 - y1, 2d));
                int dx = x2 - x1;
                int dy = y2 - y1;
            ...
            for (int i = 0; i < row.size() - 2; i++) {
                ...
                int d1 = (int) Math.sqrt(Math.pow(x2 - x1, 2d) + Math.pow(y2 - y1, 2d));
                int d2 = (int) Math.sqrt(Math.pow(x3 - x2, 2d) + Math.pow(y3 - y2, 2d));
                ...
                double u1 = Math.acos(Math.abs((double) (ax - x2) / l)) * 360 / (2 * Math.PI);
                double u2 = Math.acos(Math.abs((double) (bx - x2) / l)) * 360 / (2 * Math.PI);
                if (ax - x2 < 0) {
                    u1 = 180 - u1;
                }
                if (ay - y2 > 0) {
                ...
                g2.drawArc(x2 - l, y2 - l, 2 * l, 2 * l, (int) u1, (int) (u2 - u1));
    ...
    public static ImageIcon createImage(HyperGraph graph) {
        Image img = new BufferedImage(MAX_WIDTH, MAX_HEIGHT, BufferedImage.TYPE_INT_RGB);
    ...
    // рисуем вершины
    private static List<Point> drawVertexs(HyperGraph graph, Graphics g, int x0, int y0) {
        int r = (int) (Math.min(MAX_HEIGHT, MAX_WIDTH) / 2.5);
    ...
    private static void drawPoint(Graphics g, int x0, int y0, int r, List<Point> points, double du, int i) {
        int x;
        int y;
